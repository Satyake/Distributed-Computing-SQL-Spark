...
create table fireincidents using csv options (header 'True',
...
select * from fireincidents where `incident date` in ('4/4/2016' ,'09/27/2016');
...
select `station area` from fireincidents where `station area` > 20 and `incident date` = '04/04/2016' or `incident date`='09/27/2016';
...
-- TODO
select `ignition cause`,count(`ignition cause`) from fireincidents group by `ignition cause`;
...
-- TODO
select `ignition cause`,count(`ignition cause`) from fireincidents group by `ignition cause` order by `ignition cause`;
...
-- TODO
create table firecalls using csv  options (header 'True',
...
select * from firecalls inner join fireincidents on firecalls.`Battalion`=fireincidents.`Battalion` ;
...
-- TODO
select count(firecalls.`incident number`) from firecalls inner join fireincidents on firecalls.`Battalion` =fireincidents.`Battalion`;
